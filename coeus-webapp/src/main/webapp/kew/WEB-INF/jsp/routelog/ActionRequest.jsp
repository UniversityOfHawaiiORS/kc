...
<tr>
            ...
            <c:otherwise>
             <c:choose>
              ...
              <c:otherwise>
                ...
                <%-- When multiple found display the qualified roll before the multiple message --%>
                <c:if test="${!empty actionRequest.qualifiedRoleNameLabel}">
                            &nbsp;(<c:out value="${actionRequest.qualifiedRoleNameLabel}" />)
                </c:if>
                <%-- KC-679 END --%>
    ...
    <td align="center" class="<c:out value="${headerClass}"/>" nowrap="nowrap">
    	<c:choose>
      	    <%-- KC-679 New routing for 5.1.1 
      	         For Future Nodes....Change date column to node names (route group) so 
      	         we can tell which route together date is useless anyway --%>
            <c:when test="${!empty futureAction && futureAction == 'y'}">
                <b>&nbsp;<c:out value="${actionRequest.nodeInstance.name}"/></b>
            </c:when>
            <c:otherwise>     
