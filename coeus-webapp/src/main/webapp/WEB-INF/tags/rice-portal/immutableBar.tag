...
<div class="header2">
  <div class="header2-left-focus">
    <div class="breadcrumb-focus">
     ...
     <%-- KC-550 BEGIN Message in top bar from param --%>
      <c:set var="topbar_message" scope="session" value="<%=org.kuali.rice.coreservice.framework.CoreFrameworkServiceLocator.getParameterService().getParameterValueAsString(\"KC-GEN\", \"All\", \"uh_top_bar_message\")%>"/>
	 <c:if test="${not empty topbar_message}">
		<div id="topbar-alert">${topbar_message}</div> 
	 </c:if>
     <%--KC-550 END Message in top bar from param --%>
  </div>
</div>
<div id="login-info">
