...
<%@ attribute name="dataLength" required="false" description="If set, this will be used to determine text size limits." %>
...
<c:if test="${!readOnly}">
  <c:choose>
    ...
    <c:when test="${attributeEntry.control.text == true && empty dataLength}">
    ...
    <c:when test="${attributeEntry.control.text == true && dataLength < 100}">
        <html:text property="${property}" style="${textStyle}" title="${accessibleTitle}" tabindex="${tabindex}"
                           size="${dataLength}" maxlength="${dataLength}"
                           onblur="${onblur}" onchange="${onchange}" styleId="${property}" disabled="${disableField}"
                           styleClass="${styleClass}"/>           
    </c:when>
    ...
    <c:when test="${attributeEntry.control.text == true && dataLength >= 100}">
           <html:textarea property="${property}" style="${textStyle}" title="${accessibleTitle}" tabindex="${tabindex}"
                           ...
                           onkeyup="textLimit(this, ${dataLength});" />
           ...
           <c:if test="${dataLength > 500}">
	           ...
	           <kul:expandedTextArea textAreaFieldName="${property}" action="${fn:substringBefore(fn:substring(requestScope['org.apache.struts.taglib.html.FORM'].action, 1, -1),'.do')}" textAreaLabel="${attributeEntry.label}" disabled="${disabled}" title="${attributeEntry.label}" readOnly="${readOnly}" maxLength="${dataLength}"/>
	       </c:if>
    </c:when>
    <%-- KC-492 END --%>
